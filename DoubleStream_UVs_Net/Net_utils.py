...
def atrous_spatial_pyramid_pooling(inputs,
    ...
    aspp_features = Concatenate(name="concatenate_aspp_hsr")(branch_features)
    return aspp_features

def atrous_spatial_pyramid_pooling_sen(inputs,
                                   n_filters=256,
                                   rates=[6, 12, 18],
                                   imagelevel=True,
                                   weight_decay=1e-4,
    ...
    for i, rate in enumerate(rates):
        atrous_pool_block_i = separable_conv_bn(inputs, 256, 'ns_aspp'+str(i+1), rate=rate, depth_activation=True,
                                                weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                                                bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
        branch_features.append(atrous_pool_block_i)

    # concatenate multi-scale features
    aspp_features = Concatenate(name="concatenate_aspp_sen")(branch_features)
    return aspp_features
