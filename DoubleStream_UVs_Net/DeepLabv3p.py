...
def Deeplab_v3p_hsr(input_shape,
                    n_class,
                    encoder_name,
    ...
    net = BilinearUpSampling(target_size=(input_shape[0], input_shape[1]), name="upsam_v3p_hsr")(net)
...
def Deeplab_v3p_sen(input_shape,
                    n_class,
                    encoder_name,
                    encoder_weights=None,
                    weight_decay=1e-4,
    ...
    encoder = build_encoder(input_shape, encoder_name, encoder_weights=encoder_weights,
                            ...
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    net = encoder.get_layer(scope_table[encoder_name]["pool4"]).output
    net = atrous_spatial_pyramid_pooling_sen(net, n_filters=256, rates=[6, 12, 18], imagelevel=True,
                                         weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                                         bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    net = Conv2D(256, (1, 1), use_bias=False, activation=None, kernel_regularizer=l2(weight_decay),
                 kernel_initializer=kernel_initializer)(net)
    ...
    net = Activation("relu")(net)
    net = Dropout(0.1)(net)
    decoder_features = BilinearUpSampling(target_size=(input_shape[0] * 2, input_shape[1] * 2))(net)
    ...
    encoder_features = BatchNormalization(epsilon=bn_epsilon, momentum=bn_momentum)(encoder_features)
    encoder_features = Activation("relu")(encoder_features)
    encoder_features = BilinearUpSampling(target_size=(input_shape[0] * 2, input_shape[1] * 2))(encoder_features)
    net = Concatenate()([encoder_features, decoder_features])

    net = separable_conv_bn(net, 256, 'ns_decoder_conv1', depth_activation=True,
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    net = separable_conv_bn(net, 256, 'ns_decoder_conv2', depth_activation=True,
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    net = Dropout(0.1)(net)

    net = BilinearUpSampling(target_size=(input_shape[0] * 8, input_shape[1] * 8), name="upsam_v3p_sen")(net)
    output = Conv2D(n_class, (1, 1), activation=None, kernel_regularizer=l2(weight_decay),
                    kernel_initializer=kernel_initializer)(net)
    output = Activation("softmax")(output)
...
def trainModel_v3p_hsr(trainurls,
                       valurls,
                       hsrimage_dir,
                       label_dir,
                       nb_class,
    ...
    model_hsr = Deeplab_v3p_hsr(input_shape=(hsrimagesize, hsrimagesize, hsrchannel), n_class=nb_class, encoder_name="xception_41", encoder_weights=None,
                                weight_decay=weight_decay, kernel_initializer=kernel_initializer, bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)

    model_hsr.summary()
    model_hsr.compile(loss="binary_crossentropy", optimizer=Adam(lr=1e-4), metrics=["accuracy"])

    logs_dir = "./logsfile"
    model_name = "V3P_HSR_sz_" + str(monte_run_count) + ".h5"
    loss_name = "V3P_HSR_sz_" + str(monte_run_count) + ".csv"
...
if __name__ == "__main__":
    ...
    for monte_iter in range(1, 11):
        trainModel_v3p_hsr(trainurls=trainurls,
                           valurls=valurls,
