...
def residual_block(inputs,
    ...
    preact = Activation("relu", name=block_name+"_"+unit_name+"_act0")(preact)
    ...
    x = Activation("relu", name=block_name+"_"+unit_name+"_act1")(x)
    ...
    x = Activation("relu", name=block_name+"_"+unit_name+"_act2")(x)
...
def resnet_v2_50(input_shape,
    ...
    x = Activation("relu", name="act_encoder")(x)

    model = Model(input_x, x)
    return model


def resnet_v2_50_nosam(input_shape,
                 kernel_size=3,
                 include_root=True,
                 weight_decay=1e-4,
                 kernel_initializer="he_normal",
    ...
    if include_root:
        x = Conv2D(64, (7, 7), strides=(1, 1), padding="same", name="ns_conv1", use_bias=False,
                   activation=None, kernel_initializer=kernel_initializer,
                   kernel_regularizer=l2(weight_decay))(x)
        # x = MaxPooling2D((3, 3), (2, 2), padding="same")(x)

    x = residual_bottleneck(x, bottleneck_param(scope="ns_block1", base_depth=64, kernel_size=kernel_size,
                                                num_units=3, stride=1, rate=1),
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = residual_bottleneck(x, bottleneck_param(scope="ns_block2", base_depth=128, kernel_size=kernel_size,
                                                num_units=4, stride=1, rate=1),
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = residual_bottleneck(x, bottleneck_param(scope="ns_block3", base_depth=256, kernel_size=kernel_size,
                                                num_units=6, stride=1, rate=2),
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = residual_bottleneck(x, bottleneck_param(scope="ns_block4", base_depth=512, kernel_size=kernel_size,
                                                num_units=3, stride=1, rate=4),
                            weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                            bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)

    x = BatchNormalization(epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Activation("relu", name="ns_act_encoder")(x)
