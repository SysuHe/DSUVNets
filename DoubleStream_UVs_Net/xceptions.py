...
def xception_41_8s(input_shape,
                weight_decay=1e-4,
                kernel_initializer="he_normal",
                bn_epsilon=1e-3,
                bn_momentum=0.99):
...
def xception_41_upsam(input_shape,
                weight_decay=1e-4,
                kernel_initializer="he_normal",
                bn_epsilon=1e-3,
                bn_momentum=0.99):
    ...
    # just for sentinel-2 images
    x = UpSampling2D((8, 8))(x)

    x = Conv2D(32, (3, 3), strides=(2, 2), use_bias=False, name='ns_entry_block1_conv1', padding="same",
               kernel_initializer=kernel_initializer, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(name='ns_entry_block1_conv1_bn', epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Activation('relu', name='ns_entry_block1_conv1_act')(x)
    x = Conv2D(64, (3, 3), use_bias=False, name='ns_entry_block1_conv2', padding="same",
               kernel_initializer=kernel_initializer, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(name='ns_entry_block1_conv2_bn', epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Activation('relu', name='ns_entry_block1_conv2_act')(x)

    x = separable_residual_block(x, [128, 128, 128], "ns_entry_block2", skip_type="conv", stride=2, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = separable_residual_block(x, [256, 256, 256], "ns_entry_block3", skip_type="conv", stride=2, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = separable_residual_block(x, [728, 728, 728], "ns_entry_block4", skip_type="conv", stride=2, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)

    for i in range(1, 17):
        x = separable_residual_block(x, [728, 728, 728], "ns_middle_block"+str(i), skip_type="sum", stride=1, rate=1,
                                   weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                                   bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)

    x = separable_residual_block(x, [728, 1024, 1024], "ns_exit_block1", skip_type="conv", stride=1, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = SeparableConv2D(1536, (3, 3), strides=1, dilation_rate=2, use_bias=False, padding="same",
                        activation="relu", name="ns_exit_block2_sepconv1",
                        kernel_regularizer=l2(weight_decay), kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization(name="ns_exit_block2_sepconv1_bn", epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = SeparableConv2D(1536, (3, 3), strides=1, dilation_rate=2, use_bias=False, padding="same",
                        activation="relu", name="ns_exit_block2_sepconv2",
                        kernel_regularizer=l2(weight_decay), kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization(name="ns_exit_block2_sepconv2_bn", epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = SeparableConv2D(2048, (3, 3), strides=1, dilation_rate=2, use_bias=False, padding="same",
                        activation="relu", name="ns_exit_block2_sepconv3",
                        kernel_regularizer=l2(weight_decay), kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization(name="ns_exit_block2_sepconv3_bn", epsilon=bn_epsilon, momentum=bn_momentum)(x)

    return Model(input_x, x)

def xception_41_nosam(input_shape,
                weight_decay=1e-4,
                kernel_initializer="he_normal",
                bn_epsilon=1e-3,
                bn_momentum=0.99):
    ...
    x = Conv2D(32, (3, 3), strides=(1, 1), use_bias=False, name='ns_entry_block1_conv1', padding="same",
               kernel_initializer=kernel_initializer, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(name='ns_entry_block1_conv1_bn', epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Activation('relu', name='ns_entry_block1_conv1_act')(x)
    x = Conv2D(64, (3, 3), use_bias=False, name='ns_entry_block1_conv2', padding="same",
    ...
    x = separable_residual_block(x, [128, 128, 128], "ns_entry_block2", skip_type="conv", stride=1, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = separable_residual_block(x, [256, 256, 256], "ns_entry_block3", skip_type="conv", stride=2, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    x = separable_residual_block(x, [728, 728, 728], "ns_entry_block4", skip_type="conv", stride=1, rate=1,
                               weight_decay=weight_decay, kernel_initializer=kernel_initializer,
                               bn_epsilon=bn_epsilon, bn_momentum=bn_momentum)
    ...
    x = SeparableConv2D(2048, (3, 3), strides=1, dilation_rate=2, use_bias=False, padding="same",
                        ...
                        kernel_regularizer=l2(weight_decay), kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization(name="ns_exit_block2_sepconv3_bn", epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Conv2D(512, (1, 1), strides=(1, 1), use_bias=False, name='ns_exit_match_dim_conv', padding="same",
               kernel_initializer=kernel_initializer, kernel_regularizer=l2(weight_decay))(x)
    x = BatchNormalization(name='ns_exit_match_dim_bn', epsilon=bn_epsilon, momentum=bn_momentum)(x)
    x = Activation('relu', name='ns_exit_match_dim_act')(x)
